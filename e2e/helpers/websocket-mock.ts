...
function createMockKlineEvent(symbol = 'btcusdt') {
  return {
...
function createMockDepthEvent(symbol = 'btcusdt') {
  return {
...
function createMockTradeEvent(symbol = 'btcusdt') {
  return {
    stream: `${symbol}@trade`,
    data: {
      e: 'trade',
...
function createMockMiniTickerEvent(symbol = 'btcusdt') {
  return {
...
function createMockKlinesResponse() {
  const candles = [];
...
function createMockDepthResponse() {
  const bids: string[][] = [];
  const asks: string[][] = [];
  for (let i = 0; i < 20; i++) {
    bids.push([(67200 - i * 10).toFixed(2), (Math.random() * 5).toFixed(3)]);
    asks.push([(67210 + i * 10).toFixed(2), (Math.random() * 5).toFixed(3)]);
  }
